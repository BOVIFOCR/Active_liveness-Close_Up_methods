...
from src.CameraCloseUp.FrameSelector import FrameSelector
from src.CameraCloseUp.FeatureExtractor import FeatureExtractor
from src.CameraCloseUp.Classifier import Classifier
from src.CameraCloseUp.CloseUpLivenessDataset import CloseUpLivenessDataset

class Model:

    @staticmethod
    def preProcessDataset(df,source,faceDetector,landmarkExtractor,output,verbose=False):
        ...
        for df,label in zip([df_live,df_spoof],["live","spoof"]):
            ...
            for _,row in df.iterrows():
                try:
                    ...
                    preProcessedSample = FeatureExtractor.preProcess(samplePath,faceDetector,landmarkExtractor,device,verbose)
    ...
    @staticmethod
    def selectFrames(df,sourcePath,N,bins,faceDetector=None,landmarkExtractor=None,df_preProcessed=None,verbose=False):
        ...
        for _,sample in df.iterrows():
            try:
                ...
                selection = FrameSelector.selectFrames(samplePath,N,bins,faceDetector,landmarkExtractor,df_sample,verbose)
    ...
    @staticmethod
    def train(df_train,df_val,learningRate=0.001,batchSize=50,numEpochs=500,N=24,savePath="./cameraCloseUp_weights.pth",verbose=False):
        ...
        Utils.show("Training Camera CloseUp method!\n",verbose)
        ...
        # Instantiate model
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = Classifier(N=N,distortionFeatureSize=2088)
        model.to(device)
    ...
    @staticmethod
    def eval(df_test,N=24,weightsPath="./cameraCloseUp_weights.pth",verbose=False):
        # Load trainig and validation data
