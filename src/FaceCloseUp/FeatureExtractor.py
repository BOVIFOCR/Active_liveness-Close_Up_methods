...
class FeatureExtractor:
    ...
    @staticmethod
    def preProcess(N,samplePath,faceDetector,landmarkExtractor,verbose):
        ...
        for fileName in frames:
            ...
            if category is not None:
                features.append([fileName]+signature+[category])
            
        df_features = None
        if signatureSize is not None:
            df_features = pd.DataFrame(features,columns=["frame"]+[f"distortion_feature_{i}" for i in range(0,signatureSize)]+["category"])
